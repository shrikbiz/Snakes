...
export default class TitleEffect extends Vue {
  ...
  get secondColorEffect(): string {
    let number = Math.floor(
      Fraction(this.effectStart + 350, this.effectEnd, this.scrollVariable) *
        256
    );
    ...
    let hexValue =
      oneth?.concat(zeroth) + oneth?.concat(zeroth) + oneth?.concat(zeroth);
    return `#${hexValue}`;
  ...
  colorChange(
    ...
    let fraction = Fraction(
      this.effectStart + addToStart,
      this.effectEnd + addToEnd,
      this.scrollVariable
    );
    return [
